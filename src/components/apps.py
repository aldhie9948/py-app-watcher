...
class Apps(tk.Frame):
  ...
  def app_form_modal(self):
    ...
    type_entry = tk.Combobox(frame, values=type_combo_values, textvariable=self.type_text)
    type_entry.set(self.type_text.get())

    # form name
    tk.Label(frame, text="Name: ").grid(row=0, column=0, sticky="w", pady=2)
    tk.Entry(frame, width=70, textvariable=self.name_text).grid(row=1, column=0, pady=5, sticky="w")
    ...
    tk.Entry(frame, width=70, textvariable=self.value_text).grid(row=5, column=0, pady=5, sticky="w")
    ...
    tk.Button(frame, text="Save", command=self.submit_form).grid(row=8, column=0, pady=10)
